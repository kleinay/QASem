...
def create_csv(infile_path, outfile, transformation_model_path):
    ...
    for row in infile:
        the_roles = row["roles"].split('~!~')
        role_descriptions = row["role_descriptions"].split('~!~')
        ...
        questions = []
        roles = []
        for role, role_description in zip(the_roles, role_descriptions):
...
infile = 'Analysis/mturk/implicit/GC.test.40sampled.csv'
outfile = 'Analysis/mturk/implicit/GC.test.40sampled.baseline_qasrl.csv'
